# micro sub cci with neuron(all)
rm(list = ls())
library(Seurat)
library(CellChat)
library(tidyverse)

seu <- readRDS("rds/done_annotated.rds")
head(seu); colnames(seu@meta.data)
table(seu$celltype)


# step1 subset neuron and micro -------------------------------------------
micro_sub <- readRDS("~/Downloads/scAD/rds/micro_sub.rds")
table(Idents(micro_sub))
micro_sub$subtype <- paste0("Mg",micro_sub$seurat_clusters)

neuron_sub <- subset(seu, celltype %in% "Neuron")
neuron_sub$subtype <- "Neuron"
seu <- merge(micro_sub, neuron_sub)

Idents(seu) <- seu$group
seu <- JoinLayers(seu)  # ifelse :Error in `GetAssayData()`:! GetAssayData doesn't work for multiple layers in v5 assay.
...
# 气泡图展示中性粒细胞和内皮细胞的互作
cellchat@meta$group <- cellchat@meta$datasets
...
netVisual_bubble(cellchat,
                 sources.use = c("Mg3","Mg4"),
                 targets.use = c("Neuron"),
                 comparison = c(1,2),
                 angle.x = 45)
...
### 2.2 细胞通讯差异热图
p3 <- netVisual_heatmap(cellchat,comparison = c(1,2))
p4 <- netVisual_heatmap(cellchat, measure = "weight",comparison = c(1,2))
p3 + p4
...
netVisual_bubble(cellchat,
                 sources.use = c("Neuron"),
                 targets.use = c("Mg4","Mg1"),
                 comparison = c(1,2),
                 angle.x = 45)
